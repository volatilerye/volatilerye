            ...
            for i in range(dir_depth):
                if i == dir_depth - 1:
                    ...
                    with open(contents_path, "r") as g:
                        ...
                        contents_info += f'<a href="{"../" * (dir_depth - i - 1)}{md_dir_list[i]}.html">{info}</a> / '
...
def get_changed_files():
    ...
    for line in result.stdout.splitlines():
        _, file_path = line[:2].strip(), line[3:].strip()
...
for p in tqdm.tqdm(html_files):
    if os.path.isfile(p):
        os.remove(p)
else:
    print("🗑️ DONE: removed files.")
